...
High-churn endpoints (likes, comments, message send, feed rebuild) lack rate limits.

Add gateway or per-service throttling (e.g., token bucket in Redis) and idempotency keys for uploads/sends if needed.